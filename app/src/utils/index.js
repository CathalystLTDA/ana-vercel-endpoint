...
function probeInputFile(inputPath) {
	ffmpeg.ffprobe(inputPath, (err, metadata) => {
		if (err) {
			...
			console.log('  Streams:', metadata.streams)
		}
	})
}
...
// 			})
// 			.on('end', () => {
// 				// Delete the original file after conversion if desired
// 				try {
...
async function convertAudioToMp3(inputPath, outputPath) {
	...
	return new Promise((resolve, reject) => {
		ffmpeg(inputPath)
			...
			.on('codecData', (data) => {
				console.log('Input audio codec data:', data)
			})
			...
			.on('end', () => {
				...
				try {
					fs.unlinkSync(inputPath)
					console.log('Deleted the original file:', inputPath)
				} catch (unlinkErr) {
					console.error('Error deleting input file:', unlinkErr)
				}
